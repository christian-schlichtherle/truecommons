...
import static net.java.truecommons.cio2.Entry.Access.READ;
import static net.java.truecommons.cio2.Entry.Access.WRITE;
import net.java.truecommons.cio2.Entry.Entity;
import net.java.truecommons.cio2.Entry.Size;
...
public class MemoryBuffer implements IoBuffer {
    ...
    private final Map<Access, Long> times = new EnumMap<>(Access.class);
    private int reads;
    private int writes;
    ...
    @Override
    public Option<EntryName> name() { return Option.none(); }

    @Override
    public final Option<Long> size(final Size type) {
        if (null == buffer) return Option.none();
        return Option.some(Long.valueOf(buffer.limit()));
    }
     ...
     */
    @Override
    public final Option<Long> time(Access type) {
        return Option.apply(times.get(type));
    }
